...
module.exports = {
    ...
    execute(message, args) {
        ...
        Server.findByIdAndUpdate({_id: message.guild.id}, {"IP": args[0]}, {useFindAndModify: false})
            .catch((err) => console.error(err))

        // Remove all logs
        Log.findByIdAndUpdate({_id: message.guild.id}, { $set: { logs: [] }}, {useFindAndModify: false})
            .catch((err) => console.error(err))

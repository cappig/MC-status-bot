...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        if (!message.member.permissions.has(Permissions.FLAGS.ADMINISTRATOR) && message.member.id !== process.env.OWNERID) {
        ...
        if (args.length > 0) {
            ...
            try {
                setip.execute(message, [`${message.options.getString('ip')}`, `${message.options.getString('type')}`], client);
            } catch (error) {
                console.error(error);
                message.reply({ content: 'Uh, oh! An error occurred while trying to set the ip! (**X  _  X**)', allowedMentions: { repliedUser: false } })
            }
        ...
        if (!message.guild.me.permissions.has(Permissions.FLAGS.MANAGE_ROLES) && !message.guild.me.permissions.has(Permissions.FLAGS.MANAGE_CHANNELS)) {
            message.reply("I don't have the necessary permissions to perform this action! - `Manage roles` and `Manage channels`");
            return;
        } else if (!message.guild.me.permissions.has(Permissions.FLAGS.MANAGE_CHANNELS)) {
            message.reply("I don't have the necessary permissions to perform this action! - `Manage channels`");
            return;
        } else if (!message.guild.me.permissions.has(Permissions.FLAGS.MANAGE_ROLES)) {
            message.reply("I don't have the necessary permissions to perform this action! - `Manage roles`");
            return;
        }
        ...
        if (!ip) {
            message.reply('Please use`mc!setip` to set a ip to monitor!');
            return;
        }

    ...
    async execute(message, args) {
        ...
        if (!args[0]) {
            try {
                ...
                args = [`${message.options.getString('ip')}`, `${message.options.getString('type')}`];
                if (message.options.getString('ip') == null) {
                    console.log("areh")
                    await lookUpServer();
                }
                ip = args[0].split(':')[0].toLowerCase();
                portnum = Number(args[0].split(':')[1]);
            } catch (e) {
                console.log("an erro")
                await lookUpServer();
            }
        ...
        function online(result) {
            ...
            if (interaction) {
                message.editReply({ embeds: [embed], files: [attachment] });
            } else {
        ...
        async function lookUpServer() {
            console.log("ee")
            data = await lookup('Server', message.guild.id);

            if (!data.IP) {
                if (interaction) {
                    message.editReply('Please specify a IP address to ping!');
                } else {
                    message.reply('Please specify a IP address to ping!');
                }
                return;
            }
            if (data.Bedrock == true) bedrock = true;
            ip = data.IP.split(':')[0];
            portnum = Number(data.IP.split(':')[1]);
            return;
        }
